import type { Severity } from '@asm/shared';
...
export interface SecurityHeadersCheckResult {
  ...
  missing: string[];
  missingDetails: SecurityHeader[];
  present: Array<{ name: string; short: string; value: string }>;
}

export const REQUIRED_HEADERS: SecurityHeader[] = [
  {
...
export async function checkSecurityHeaders(assetValue: string): Promise<SecurityHeadersCheckResult> {
  ...
  for (const url of urls) {
    try {
      ...
      clearTimeout(timer);
      rawHeaders = Object.fromEntries([...res.headers.entries()].map(([k, v]) => [k.toLowerCase(), v]));
      checkedUrl = url;
      break;
    } catch {
      continue;
  ...
  const missingHeaders = REQUIRED_HEADERS.filter((h) => !rawHeaders![h.name.toLowerCase()]);
  ...
  return {
    ...
    checkedUrl,
    headers: rawHeaders,
    missing: missingHeaders.map((h) => h.short),
    missingDetails: missingHeaders,
    present,
  };
}
